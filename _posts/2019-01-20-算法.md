...
　　Szegedy等人首次证明了可以通过对图像添加人类察觉不到的极小扰动来误导神经网络做出误分类。他们的思路是尝试求解让神经网络做出误分类的最小扰动$\rho$的方程如下：&\underset{\rho}{min}\left \| \rho  \right \|\_{2} s.t. C(I\_{c}+\rho )=l;I\_{c}+\rho \in  [0,1]^{m}&其中，$I\_{c} \in \mathbb{R}^{m}$指原始图片，下标指明图片是未经过修改的；$C(.)$是指神经网络分类器；$l$指图像的标签。但由于问题的复杂度太高，他们简化了问题，即寻找最小的损失函数添加项，使得神经网络做出误分类，将问题转化成了凸优化过程：&\underset{\rho}{c\left | \rho  \right |+L(I\_{c}+\rho ,l)} s.t.I\_{c}+\rho \in  [0,1]^{m}&其中，$L(.,.)$计算分类器的损失值。
