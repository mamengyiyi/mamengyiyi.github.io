...
对应$\epsilon$-greedy Policy Improvement来说，假设$m$个动作都以非零的$\epsilon/m$的概率被探索，$1-\epsilon$的概率选择贪心动作，那么@$\pi(a | s)=\left\{\begin{array}{ll}{\epsilon / m+1-\epsilon} & {\text { if } a^{\*}=\underset{a \in \mathcal{A}}{\operatorname{argmax}} Q(s, a)} \\ {\epsilon / m} & {\text { otherwise }}\end{array}\right.$@
