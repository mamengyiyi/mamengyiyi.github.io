...
事实上，VDN可行的原因是满足如下条件：$$\underset{\mathbf{u}}{\operatorname{argmax}} Q_{t o t}(\tau, \mathbf{u})=\left(\begin{array}{c}{\operatorname{argmax}_{u^{1}} Q_{1}\left(\tau^{1}, u^{1}\right)} \\ {\vdots} \\ {\operatorname{argmax}_{u^{n}} Q_{n}\left(\tau^{n}, u^{n}\right)}\end{array}\right)$$
...
QMIX的优化目标是最小化如下loss：$$\mathcal{L}(\theta)=\sum_{i=1}^{b}\left[\left(y_{i}^{t o t}-Q_{t o t}(\tau, \mathbf{u}, s ; \theta)\right)^{2}\right]$$
