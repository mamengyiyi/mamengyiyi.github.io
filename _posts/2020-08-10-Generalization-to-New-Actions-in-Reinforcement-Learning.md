...
本文的目标是在给定的动作集$\mathbb{A}=\left\{a\_{1}, \ldots, a\_{N} \right\}$上训练，在一个训练中未见过的动作集$\mathbb{A}^{\prime}$中采样的动作子集$\mathcal{A} \subset \mathbb{A}^{\prime}$上进行测试，以最大化该动作子集$\mathcal{A}$上的累积收益：
...
给定动作集$\mathcal{A}=\left\{a\_{1}, \ldots, a\_{k} \right\}$与对应的动作表征$\left\{c\_{1}, \ldots, c\_{k} \right\}$作为策略网络的输入，计算每个动作在当前状态下的得分，再经过softmax得到动作的概率分布：
