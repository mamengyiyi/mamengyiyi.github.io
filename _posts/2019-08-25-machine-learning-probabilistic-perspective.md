...
无监督学习和监督学习不同的包括：
* 非监督学习的模型为$p\left(\mathbf{x}\_{i} \| \boldsymbol{\theta}\right)$，是unconditional density estimation。监督学习的模型为$p\left(y_{i} \| \mathbf{x}\_{i}, \boldsymbol{\theta}\right)$，是conditional density estimation
...
解决维度诅咒的主要方法是对数据分布的性质做一些假设，即归纳偏置（inductive bias）。这种假设可以表示为具有固定数量参数的统计模型，即参数模型（parametric model）。例如线性回归和逻辑回归。

## 1.4.5 Linear regression
线性回归可以表示为
@y(\mathbf{x})=\mathbf{w}^{T} \mathbf{x}+\epsilon=\sum_{j=1}^{D} w_{j} x_{j}+\epsilon@
其中$\mathbf{w}^{T} \mathbf{x}$表示输入向量$\mathbf{x}$和模型权重向量$\mathbf{w}^{T}$之间的内积或标量积，$\epsilon$表示预测值和真实值之间的残差（residual error）。通常，我们假设$\epsilon$服从高斯分布，即$\mathcal{N}\left(\mu, \sigma^{2}\right)$。其中$\mu$是均值，$\sigma^{2}$是方差。<br>

基于以上假设，可以将线性回归模型表示为高斯分布：@p(y | \mathbf{x}, \boldsymbol{\theta})=\mathcal{N}\left(y | \boldsymbol{\mu}(\mathbf{x}), \sigma^{2}(\mathbf{x})\right)@
这个表示可以理解为：因为$\epsilon$服从高斯分布，而$\mathbf{w}^{T} \mathbf{x}$为一个常数（均值），因此$y(\mathbf{x})$也服从高斯分布。举例来说，假设$\boldsymbol{\mu}$是关于$\mathbf{x}$的线性方程$\mathbf{w}^{T} \mathbf{x}$，$\sigma^{2}(x) = \sigma^{2}$固定。进一步假设输入是一维的，则$\boldsymbol{\mu}(\mathbf{x})=w_{0}+w_{1}x=\mathbf{w}^{T} \mathbf{x}$，其中$w_{0}$就是$\epsilon$，而$\mathbf{x})=(1,x)$。<br>
同理，通过将$\mathbf{x}$替换为输入的某些非线性函数，可以对线性回归建模非线性关系@p(y | \mathbf{x}, \boldsymbol{\theta})=\mathcal{N}\left(y | \mathbf{w}^{T} \boldsymbol{\phi}(\mathbf{x}), \sigma^{2}\right)@
其中，$\phi(\mathbf{x})=\left[1, x, x^{2}, \ldots, x^{d}\right]$，这称为多项式回归（polynomial regression.）。<br>
注：在输入向量之前加上一个常数1项是一个常见的符号技巧，它允许我们将截距项与模型中的其他项组合起来。


## 1.4.6 Logistic regression
我们可以通过进行两步修改将线性回归推广到二元分类回归，即逻辑回归。首先，将高斯分布替换为伯努利分布。@p(y | \mathbf{x}, \mathbf{w})=\operatorname{Ber}(y | \mu(\mathbf{x}))@
其中$\mu(\mathbf{x})=\mathbb{E}[y | \mathbf{x}]=p(y=1 | \mathbf{x})$
其次，我们像在线性回归中一样，计算输入的线性组合，但是通过一个sigmoid函数定义$\mu(\mathbf{x})=\operatorname{sigm}\left(\mathbf{w}^{T} \mathbf{x}\right)$来确保$0 \leq \mu(\mathbf{x}) \leq 1$。sigmoid也被称为压缩函数（squashing function），因为它将整个实数空间映射到$[0,1]$，因此可以将输出解释为概率。通过这两步，可以得到逻辑回归模型的概率表示@p(y | \mathbf{x}, \mathbf{w})=\operatorname{Ber}\left(y | \operatorname{sigm}\left(\mathbf{w}^{T} \mathbf{x}\right)\right)@
在实际中，可以规定一个阈值，若概率大于这个阈值，则输出为1，否则为0。

## 1.4.7 Overfitting
当我们调参模型时，我们需要注意不要过度拟合数据，也就是说，我们应该避免尝试对输入中的每个微小变化进行建模，因为这变化更可能是噪声。

## 1.4.8 Model selection
为了避免过拟合的问题，我们可以通过最小化验证集（validation set）上的泛化误差（generalization error）来选择适合的模型。通常的做法是交叉验证（cross validation）：我们将训练数据分成$K$个部分; 然后，对于每个部分$k \in {1 , ... , K}$，我们在除了第$k$部分之外的所有部分上训练，并以循环方式测试，最后，我们计算在所有轮次上平均的误差，并将其用作泛化误差。如果$K=N$，那么这种方式叫做留一交叉验证（leave-one out cross validation）。

## 1.4.9 no free lunch theorem
> All models are wrong, but some models are useful. – George Box
许多机器学习都涉及设计不同的模型，以及适合它们的不同算法。我们可以使用交叉验证等方法来根据经验为我们的特定问题选择最佳方法。但是，不存在普遍最好的模型。因此，我们需要设计许多不同类型的模型，以涵盖现实世界中出现的各种数据。
