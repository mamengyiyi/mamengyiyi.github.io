...
基于learned skills对expert policy进行模仿学习，假如给定一个专家的state trajectory $\tau^{\*}=\\{(s_{i})\\}\_{1 \leq i \leq N}$，使用训练好的discriminator对最优的可能生成这样的trajectory的skill进行esimate，然后返回control policy进行imitation。这样的imitation成功模仿了4个expert中的3个：
